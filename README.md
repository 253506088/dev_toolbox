...
| 二维码 | 生成二维码图片（解析功能暂不支持 Windows 桌面端） |
...
2. SQL 格式化。输入 SQL 语句，支持【格式化】（美化缩进）和【压缩】（去除多余空格换行）功能。

3. JSON 格式化。输入一串 JSON 字符串，提供如下按钮的功能【格式化】、【压缩】、【转义】、【去转义】、【Unicode编码】、【Unicode解码】、【复制】、【清空】

4. 时间与时间戳相互转换，用户输入【YYYY-MM-DD HH:mm:ss】格式的时间（也可以通过时间组件选择），转换为毫秒级和秒级的时间戳，相反输入时间戳（秒级或毫秒的）转换为【YYYY-MM-DD HH:mm:ss】格式的时间
...
8. 二维码生成 & 二维码解析。输入文本内容转换为二维码。粘贴入二维码图片或通过上传二维码图片，解析二维码的内容。

9. Cron 表达式。包含秒、分、时、日、月、周、年的 Tab 页，每个 Tab 支持：每X（通配符*）、周期（范围-）、从X开始每Y执行（步长/）、指定（勾选具体值）。表达式可以转换为 GUI 选项，GUI 选项也可以转换为表达式。

10. XML 转 JSON，JSON 转 XML

11. 文本对比。类似于 Git 的对比功能，支持左右分栏显示，自动识别并高亮行级差异（新增/删除），且支持行内字符级别的细节差异高亮（如仅修改了某个单词或标点）。

注意：这 11 个功能，请按照功能拆分成不同的文件，避免一个文件里塞入了十一个功能。
```
